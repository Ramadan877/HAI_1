from flask import Flask, request, render_template, jsonify, session, send_from_directory, send_file, Response
...
def data_dashboard():
    """Research data dashboard with User_Data structure export"""
    try:
        ...
        recent_participants = Participant.query.order_by(Participant.id.desc()).limit(10).all()
        
        html = f"""
        <html>
        <head>
            <title>HAI Research Data - V1</title>
            <style>
                ...
                .export-buttons a:hover {{ background-color: #005a87; }}
            </style>
        </head>
        <body>
            <h1>HAI Research Data Dashboard - Version 1</h1>
            
            <h2>📊 Summary Statistics</h2>
            <ul>
                <li><strong>Total Participants:</strong> {total_participants}</li>
                <li><strong>Total Sessions:</strong> {total_sessions}</li>
                <li><strong>Total Interactions:</strong> {total_interactions}</li>
                <li><strong>Total Recordings:</strong> {total_recordings}</li>
            </ul>
            
            <div class="export-buttons">
                <h2>📥 Data Export Options</h2>
                <a href="/export_complete_data">📦 Download Complete Data (ZIP)</a>
                <a href="/export_csv">📊 Download Data as CSV</a>
                <a href="/browse_files">📁 Browse Uploaded Files</a>
            </div>
            ...
            <table>
                <tr>
                    ...
                    <th>Interactions</th>
                    <th>Created</th>
                    <th>Export Individual</th>
                </tr>
        """
        
        for participant in recent_participants:
            sessions_count = Session.query.filter_by(participant_id=participant.id).count()
            interactions_count = db.session.query(Interaction)\
                .join(Session)\
                .filter(Session.participant_id == participant.id).count()
            
            html += f"""
                <tr>
                    <td>{participant.id}</td>
                    <td>Age: {participant.age or 'N/A'}, Gender: {participant.gender or 'N/A'}</td>
                    <td>{sessions_count}</td>
                    <td>{interactions_count}</td>
                    <td>{participant.created_at or 'N/A'}</td>
                    <td><a href="/export_participant_data/{participant.id}">Export P{participant.id}</a></td>
                </tr>
            """
...
def export_complete_data():
    """Export all data maintaining User_Data folder structure"""
    try:
        import zipfile
        from datetime import datetime
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        export_folder = os.path.join('exports', f'HAI_V1_Complete_Export_{timestamp}')
        os.makedirs(export_folder, exist_ok=True)
        
        # 1. Export database data as CSV files
        db_folder = os.path.join(export_folder, 'Database_Export')
        os.makedirs(db_folder, exist_ok=True)
        export_database_to_csv(db_folder)
        
        # 2. Copy User_Data structure exactly as is
        user_data_source = os.path.join('uploads', 'User Data')
        user_data_dest = os.path.join(export_folder, 'User_Data')
        
        if os.path.exists(user_data_source):
            shutil.copytree(user_data_source, user_data_dest)
            
        # 3. Create README file
        readme_content = f"""HAI Research Data Export - Version 1
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

Structure:
- Database_Export/: All database tables as CSV files
- User_Data/: Participant folders with audio recordings and files
  └── [Participant_ID]/: Individual participant folders
      ├── *.mp3: Audio recordings
      ├── *.wav: Audio files  
      ├── *.txt: Conversation logs
      └── Screen Recordings/: Screen recording files

Database Tables:
- participants.csv: Participant demographics
- sessions.csv: Study sessions data
- interactions.csv: AI conversation data
- recordings.csv: Audio file metadata
- user_events.csv: User interface interactions

Total Participants: {Participant.query.count()}
Total Sessions: {Session.query.count()}
Total Interactions: {Interaction.query.count()}
"""
        ...
        return send_file(zip_path, as_attachment=True, 
                        download_name=f'HAI_V1_Complete_Export_{timestamp}.zip')
        
    except Exception as e:
        logger.error(f"Export failed: {str(e)}")
        return jsonify({'error': f'Export failed: {str(e)}'}), 500

def export_database_to_csv(db_folder):
    """Export all database tables to CSV files"""
    import csv
    
    # Export Participants
    participants = Participant.query.all()
    with open(os.path.join(db_folder, 'participants.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'age', 'gender', 'education', 'native_language', 'english_proficiency', 'created_at'])
        for p in participants:
            writer.writerow([p.id, p.age or '', p.gender or '', p.education or '', 
                           p.native_language or '', p.english_proficiency or '', p.created_at or ''])
    
    # Export Sessions
    sessions = Session.query.all()
    with open(os.path.join(db_folder, 'sessions.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['session_id', 'participant_id', 'trial_type', 'version', 'start_time', 'end_time'])
        for s in sessions:
            writer.writerow([s.session_id or '', s.participant_id or '', s.trial_type or '', 
                           s.version or 'V1', s.start_time or '', s.end_time or ''])
    
    # Export Interactions
    interactions = Interaction.query.all()
    with open(os.path.join(db_folder, 'interactions.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'session_id', 'speaker', 'concept_name', 'message', 'attempt_number', 'timestamp'])
        for i in interactions:
            writer.writerow([i.id or '', i.session_id or '', i.speaker or '', i.concept_name or '', 
                           i.message or '', i.attempt_number or '', i.timestamp or ''])
    
    # Export Recordings
    recordings = Recording.query.all()
    with open(os.path.join(db_folder, 'recordings.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'session_id', 'recording_type', 'file_path', 'original_filename', 
                        'file_size', 'concept_name', 'attempt_number', 'created_at'])
        for r in recordings:
            writer.writerow([r.id or '', r.session_id or '', r.recording_type or '', r.file_path or '', 
                           r.original_filename or '', r.file_size or '', r.concept_name or '', 
                           r.attempt_number or '', r.created_at or ''])

@app.route('/browse_files')
def browse_files():
    """Browse uploaded files maintaining User_Data structure"""
    try:
        user_data_path = os.path.join('uploads', 'User Data')
        
        html = """
        <html>
        <head>
            <title>File Browser - HAI V1</title>
            <style>
                body { font-family: Arial, sans-serif; margin: 40px; }
                .participant-folder { margin: 20px 0; padding: 15px; border: 1px solid #ddd; border-radius: 5px; }
                .file-list { margin-left: 20px; }
                .file-item { margin: 5px 0; }
                .file-item a { text-decoration: none; color: #007cba; }
                .file-item a:hover { text-decoration: underline; }
                .file-size { color: #666; font-size: 0.9em; }
            </style>
        </head>
        <body>
            <h1>📁 File Browser - HAI Research V1</h1>
            <p><a href="/data_dashboard">← Back to Dashboard</a></p>
        """
        
        if os.path.exists(user_data_path):
            participant_folders = sorted([d for d in os.listdir(user_data_path) 
                                        if os.path.isdir(os.path.join(user_data_path, d))])
            
            for participant_id in participant_folders:
                participant_path = os.path.join(user_data_path, participant_id)
                html += f'<div class="participant-folder"><h3>👤 Participant {participant_id}</h3>'
                ...
                for root, dirs, files in os.walk(participant_path):
                    for file in files:
                        file_path = os.path.join(root, file)
                        relative_path = os.path.relpath(file_path, user_data_path)
                        file_size = os.path.getsize(file_path)
                        folder_name = os.path.basename(os.path.dirname(file_path))
                        
                        if folder_name == participant_id:
                        ...
                        html += f'''
                        <div class="file-item">
                            📄 <a href="/download_user_file/{relative_path.replace(os.sep, "/")}" target="_blank">{file}</a>
                            <span class="file-size">({file_size:,} bytes - {folder_display})</span>
                        </div>
                        '''
                
                html += '</div></div>'
        else:
            html += '<p>No uploaded files found.</p>'
        
        html += '</body></html>'
        return html
        
    except Exception as e:
        return f"<h1>Error browsing files</h1><p>{str(e)}</p>"

@app.route('/download_user_file/<path:filepath>')
def download_user_file(filepath):
    """Download individual file from User_Data structure"""
    try:
        full_path = os.path.join('uploads', 'User Data', filepath)
...
@app.route('/export_participant_data/<int:participant_id>')
def export_participant_data(participant_id):
    """Export data for a specific participant maintaining structure"""
    try:
        import zipfile
        from datetime import datetime
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        export_folder = os.path.join('exports', f'Participant_{participant_id}_V1_{timestamp}')
        os.makedirs(export_folder, exist_ok=True)
        
        # Export participant's database data
        db_folder = os.path.join(export_folder, 'Database_Data')
        os.makedirs(db_folder, exist_ok=True)
        export_participant_database_data(participant_id, db_folder)
        
        # Copy participant's User_Data folder structure
        participant_source = os.path.join('uploads', 'User Data', str(participant_id))
        participant_dest = os.path.join(export_folder, f'Participant_{participant_id}_Files')
        
        if os.path.exists(participant_source):
            shutil.copytree(participant_source, participant_dest)
        ...
        return send_file(zip_path, as_attachment=True, 
                        download_name=f'Participant_{participant_id}_V1_Data.zip')
        
    except Exception as e:
        return jsonify({'error': f'Export failed: {str(e)}'}), 500

def export_participant_database_data(participant_id, db_folder):
    """Export database data for specific participant"""
    import csv
    ...
    interactions = Interaction.query.filter(Interaction.session_id.in_(session_ids)).all()
    recordings = Recording.query.filter(Recording.session_id.in_(session_ids)).all()
    user_events = UserEvent.query.filter(UserEvent.session_id.in_(session_ids)).all()
    
    # Export participant info
    if participant:
        with open(os.path.join(db_folder, f'participant_{participant_id}.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'age', 'gender', 'education', 'created_at'])
            writer.writerow([participant.id, participant.age or '', participant.gender or '', 
                           participant.education or '', participant.created_at or ''])
    
    # Export sessions
    with open(os.path.join(db_folder, f'sessions_{participant_id}.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['session_id', 'participant_id', 'trial_type', 'start_time', 'end_time'])
        for s in sessions:
            writer.writerow([s.session_id, s.participant_id, s.trial_type or '', s.start_time or '', s.end_time or ''])
    
    # Export interactions
    with open(os.path.join(db_folder, f'interactions_{participant_id}.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'session_id', 'speaker', 'concept_name', 'message', 'attempt_number', 'timestamp'])
        for i in interactions:
            writer.writerow([i.id, i.session_id, i.speaker or '', i.concept_name or '', 
                           i.message or '', i.attempt_number or '', i.timestamp or ''])

@app.route('/export_csv')
def export_csv():
    """Export complete database as single CSV file"""
    try:
        ...
        # Headers
        writer.writerow(['participant_id', 'age', 'gender', 'education', 'session_id', 'trial_type', 
                        'interaction_id', 'speaker', 'concept', 'message', 'attempt_number', 'timestamp'])
        
        # Get all data with joins
        query = db.session.query(Participant, Session, Interaction)\
            .join(Session, Participant.id == Session.participant_id)\
            .join(Interaction, Session.session_id == Interaction.session_id)\
            .order_by(Participant.id, Interaction.timestamp)
        
        for participant, session, interaction in query:
            writer.writerow([
                participant.id,
                participant.age or '',
                participant.gender or '',
                participant.education or '',
                session.session_id,
                session.trial_type or '',
                interaction.id,
                interaction.speaker or '',
                interaction.concept_name or '',
                interaction.message or '',
                interaction.attempt_number or '',
                interaction.timestamp or ''
            ])
        ...
        return Response(
            output.getvalue(),
            mimetype='text/csv',
            headers={'Content-Disposition': 'attachment; filename=HAI_V1_Research_Data.csv'}
        )
